...
- `on_update` `(string: "require_healthy")` - Specifies how the check should be
  evaluated when determining deployment health. Value is inherited from the
  check's service if not set.

  - `require_healthy` - In order to be considered healthy, the check must
    report as healthy.
